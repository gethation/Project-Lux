"""Real USD/TWD from Twelve Data.

UMC is priced in actual dollars, so this pair needs an actual USD/TWD rate.
BitoPro's USDT/TWD -- correct for a pair whose US leg is USDT-denominated -- is
a different rate carrying a 0.26% Taiwan-local crypto premium that no
combination of crypto pairs removes, measured at 77% of the spread's own
standard deviation. See docs/CCF_UMC_PLAN.md.
...
The quotes carry no bid/ask, and nothing downstream wants one: the FX rate is
the reference that makes a TWD future comparable to a USD equity, not a book
this strategy crosses. core.tradable_spread prices it at its mid in both
