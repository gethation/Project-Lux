...
class IbkrReadOnlyBroker:
    ...
    the same LUX_READONLY_BROKER environment variable the Fubon read-only broker
    uses, so one switch governs both venues.

    UnrealizedPnL is carried through in the margin snapshot's raw payload so the
    account panel can show the UMC leg's open PnL; IBKR reports it under a
    different tag than the Binance fields the panel used to read.
    """
    ...
    def fetch_snapshot(self) -> BrokerAccountSnapshot:
        ...
        margins = (
            BrokerMarginSnapshot(
                ...
                # Flattened alongside the full list so the account panel can read
