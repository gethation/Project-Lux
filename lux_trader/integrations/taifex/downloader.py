...
from ...market_data.session import ccf_symbol_to_taifex_contract_month
...
class TaifexCcfTradeDownloader:
    def __init__(
        self,
        cache_dir: Path,
        *,
    ...
    def fetch_1m(self, symbol: str, start: datetime, end: datetime) -> pd.DataFrame:
        ...
        contract_month = ccf_symbol_to_taifex_contract_month(
            symbol,
        ...
        for trading_date in calendar_dates(start.date(), end.date() + timedelta(days=1)):
            ...
            if entry is None:
                continue
            frames.append(self._read_ccf_1m(entry, contract_month))
        ...
        with ZipFile(io.BytesIO(archive)) as zip_file:
            ...
            for csv_name in csv_names:
                with zip_file.open(csv_name) as csv_file:
                    rows.extend(parse_taifex_ccf_tick_csv(csv_file, contract_month))
...
def parse_taifex_ccf_tick_csv(
    ...
    for chunk in pd.read_csv(
        ...
        filtered = chunk.loc[
            (chunk["商品代號"].astype(str).str.strip() == "CCF")
