...
# Which of the two weekend rules a pair applies to the last trading session of each
# ISO week. Mirrors the PoC's ``--weekend-policy`` so backtest and live share one
# vocabulary (see the PoC's ``docs/ccf_umc_weekend_policy.md``).
#
#   flat      no entries in that session, and force-close on its final bar
#   no-entry  keep the entry ban, drop the force-close
#   none      neither rule
#
# QFF/TSM needs ``flat``: Binance perpetuals trade 24/7 while TAIFEX freezes, so an
# open position is an uncovered leg over the weekend. CCF/UMC has no such structure
# -- TAIFEX and NYSE both close -- so it runs ``none``.
...
def is_weekend_force_exit_bar(
    ...
    *,
    ...
    Pairs whose ``weekend_policy`` is not ``flat`` never force-close, so this
...
class TaifexSessionCalendar:
    ...
    def __init__(self, weekend_policy: str = WEEKEND_POLICY_FLAT) -> None:
    ...
    def annotate(self, bars: Iterable[MarketBar]) -> list[MarketBar]:
        ...
        # Both rules key off the same week-end detection, so it is computed once and
        # then gated separately -- `no-entry` still needs the session set even though
        # it drops the force-close.
