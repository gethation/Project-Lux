...
class FeeConfig:
    umc_fee_bps: float
    ccf_fee_per_contract_twd: float
    ccf_tax_rate: float
...
class LiveMarketDataConfig:
    ...
    # Max share of warmup minutes allowed to be CCF forward-filled before the
    # warmup is treated as too degraded to trade on. 1.0 disables the gate.
    warmup_forward_fill_max_ratio: float = 0.9
    # Max consecutive CCF trading minutes that may be forward-filled at the
...
class BrokerReconciliationConfig:
    ...
    umc_units_tolerance: float
    ccf_contract_tolerance: int
...
def load_config(path: Path) -> AppConfig:
    ...
    return AppConfig(
        ...
        broker_reconciliation=BrokerReconciliationConfig(
            ...
            umc_units_tolerance=float(
                broker_reconciliation.get("umc_units_tolerance", 1e-6)
            ),
            ccf_contract_tolerance=int(
                broker_reconciliation.get("ccf_contract_tolerance", 0)
            ),
        ),
        live_execution=LiveExecutionConfig(
        ...
        live_execution_smoke=LiveExecutionSmokeConfig(
            ...
            umc_units=optional_positive_float(
                live_execution_smoke.get(
                    "umc_units",
                    live_execution.get("smoke_test_umc_units", 0.1),
                ),
                "live_execution_smoke.umc_units",
            )
            or 0.1,
