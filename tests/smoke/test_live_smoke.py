...
def test_live_marketdata_providers_fetch_quotes_and_ccf_candles() -> None:
    config = load_smoke_config()
    ...
    try:
        ...
        if config.live.ccf_symbol.lower() == "auto" and config.contract_policy.enabled:
            assert contract.expiry is not None
            assert contract.policy_state == "active"

        ccf_quote = ccf.fetch_quote(contract.symbol)
        assert ccf_quote.price > 0
        if ccf_quote.bid is not None or ccf_quote.ask is not None:
            assert ccf_quote.bid is not None
            assert ccf_quote.ask is not None
            assert ccf_quote.bid > 0
            assert ccf_quote.ask > 0

        end = floor_minute(datetime.now(TAIPEI_TZ)) - timedelta(minutes=1)
        start = end - timedelta(days=7)
        ccf_candles = ccf.fetch_1m(contract.symbol, start, end)
        assert not ccf_candles.empty
        assert ccf_candles["close"].notna().all()
    finally:
        ccf.close()
...
def test_ccf_warmup_check_smoke_uses_fubon_and_taifex_network() -> None:
    config = load_smoke_config()
    result = CcfWarmupCheckRunner(config).run(output_csv="")
...
def test_warmup_live_smoke_writes_seed_bars_only() -> None:
    ...
    try:
        ...
        null_count = connection.execute(
            ...
            SELECT COUNT(*)
            FROM warmup_bars
            WHERE ccf_close_filled IS NULL
               OR umc_twd_fair IS NULL
               OR spread IS NULL
            """
        ).fetchone()[0]
        assert null_count == 0
    ...
    try:
        ...
        null_count = connection.execute(
            ...
            WHERE ccf_close_filled IS NULL
               OR umc_twd_fair IS NULL
               OR spread IS NULL
               OR ccf_symbol IS NULL
            """
        ).fetchone()[0]
        assert null_count == 0
        selected_symbols = connection.execute(
            "SELECT DISTINCT ccf_symbol FROM warmup_bars"
        ).fetchall()
        assert selected_symbols == [(first_result.ccf_symbol,)]
