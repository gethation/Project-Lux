...
def worker(
    ...
    *,
    ...
    return _IbkrWorkerClient(
        config or IbkrConnectionConfig(client_id=17_555),
...
def test_the_subscription_is_opened_once_and_kept() -> None:
    ...
    # Type 1 = live. Requesting 3 would OVERRIDE a real-time entitlement, so an
    # unentitled session must fail loudly instead of quietly serving delayed
    # data -- which has no book, so the directional signal could never fire.
    assert ib.market_data_type_calls == [1]


def test_market_data_type_is_configurable_for_offline_work() -> None:
    """Delayed data stays reachable on purpose, but only by asking for it."""
    from lux_trader.integrations.ibkr.client_process import (
    ...
    client = worker(
        ib,
        config=IbkrConnectionConfig(market_data_type=DELAYED_MARKET_DATA_TYPE),
    )
    client.fetch_umc_quote(quote_wait_timeout_seconds=5.0)

    assert ib.market_data_type_calls == [DELAYED_MARKET_DATA_TYPE]


def test_an_unknown_market_data_type_is_rejected_at_construction() -> None:
    from lux_trader.integrations.ibkr.client_process import IbkrConnectionConfig

    with pytest.raises(ValueError, match="market_data_type must be one of"):
        IbkrConnectionConfig(market_data_type=7)
