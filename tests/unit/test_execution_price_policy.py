...
def pair_plan(direction: Direction, timestamp: datetime):
    if direction == Direction.SHORT_US_LONG_TW:
        us_leg_side = OrderSide.SELL
        tw_leg_side = OrderSide.BUY
    else:
        us_leg_side = OrderSide.BUY
        tw_leg_side = OrderSide.SELL
...
def test_short_entry_price_policy_uses_sell_bid_and_buy_ask() -> None:
    ...
    plan = apply_live_touch_market_price_policy(
        pair_plan(Direction.SHORT_US_LONG_TW, timestamp),
        quote_set(timestamp),
    ...
    tw_leg = leg_by_broker(plan, BrokerName.FUBON)
    ...
    assert us_leg.expected_price == 600.0
    assert us_leg.price == 600.0
    assert us_leg.trigger_bid == 600.0
    assert us_leg.trigger_ask == 651.0
    assert us_leg.raw["accounting_price"] == 125.0
    assert us_leg.raw["us_leg_contract_multiplier"] == 5.0
    assert tw_leg.expected_price == 101.0
    assert tw_leg.price == 101.0
    assert tw_leg.trigger_bid == 99.0
    assert tw_leg.trigger_ask == 101.0


def test_long_entry_price_policy_uses_buy_ask_and_sell_bid() -> None:
    timestamp = ts("2026-06-22T09:00:00+08:00")
    plan = apply_live_touch_market_price_policy(
        pair_plan(Direction.LONG_US_SHORT_TW, timestamp),
        quote_set(timestamp),
        max_plan_age_seconds=120,
        us_leg_contract_multiplier=5.0,
    )
    ...
    us_leg = leg_by_broker(plan, BrokerName.BINANCE)
    tw_leg = leg_by_broker(plan, BrokerName.FUBON)
    assert us_leg.expected_price == 651.0
    assert us_leg.price == 651.0
    assert tw_leg.expected_price == 99.0
    assert tw_leg.price == 99.0
