...
@pytest.mark.parametrize(
    ("direction", "label"),
    [
        (Direction.LONG_UMC_SHORT_CCF, "LONG"),
        (Direction.SHORT_UMC_LONG_CCF, "SHORT"),
    ],
)
def test_live_terminal_reporter_labels_open_state_by_umc_direction(
    direction: Direction,
    label: str,
) -> None:
    stream = io.StringIO()
...
def test_live_terminal_reporter_clears_live_before_permanent_lines() -> None:
    ...
    reporter.warn(ts("2026-06-18T09:13:23+08:00"), "stale_umc", "skipped_minute")
...
def test_tradable_spread_uses_ccf_specific_stale_threshold() -> None:
    ...
    stale_ccf = estimate_tradable_spreads(
        LiveQuoteSet(
    ...
    assert stale_ccf.short_spread is None
    assert stale_ccf.long_spread is None
...
def test_tradable_spread_keeps_umc_and_usd_twd_at_global_stale_threshold() -> None:
    ...
    stale_umc = estimate_tradable_spreads(
        LiveQuoteSet(
...
def test_tradable_spread_treats_ccf_diagnostic_quote_as_stale_ccf() -> None:
    indicator = IndicatorEngine(window=3)
    for spread in (1.0, 2.0, 3.0):
        indicator.update(bar(spread))
    observed_at = ts("2026-06-18T09:00:10+08:00")
    ccf_diagnostic_quote = LiveQuote(
        source="fubon_ccf",
        symbol="CCF",
    ...
    assert snapshot.short_spread is None
    assert snapshot.long_spread is None
    assert snapshot.missing_reason == "stale_ccf"
