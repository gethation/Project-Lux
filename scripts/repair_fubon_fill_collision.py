...
def normalized_order(order: dict[str, Any], *, order_id: str) -> tuple[Any, ...]:
    ...
    return (
        ...
        request["price"],
        order["status"],
        request.get("ccf_symbol"),
        request.get("ccf_expiry"),
...
def main() -> int:
    ...
    try:
        ...
        expected = float(state.get("ccf_contracts") or 0.0)
        print(f"store={store_path}")
        print(f"state=paused expected_ccf={expected:g} recorded_ccf={before:g}")
        print(
            "repair="
                ...
                json.dumps(
                    {
                        ...
                        "recorded_ccf_before": before,
                        "recorded_ccf_after": after,
        ...
        print(f"applied=passed recorded_ccf={after:g}")
        return 0
    except Exception:
        connection.rollback()
        raise
