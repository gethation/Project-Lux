...
**⚠ 本段的原始內容已於 2026-08-04 過時**（原文：放空／借券／recall 需 margin
帳戶才能驗證）。margin 帳戶與借券已於 08-03 確認，**單腿實單多空各一次來回已於
08-04 跑通**（`position=-1`，借券實際交割）。**仍然無法驗證的只剩**：兩腿同時、
`unknown` → PAUSE、以及 recall（無法主動製造）。詳見 §6 的 E4-prep 結果。
**測試全綠不等於可以上線** —— 這句話在 08-04 得到實證：586 個測試全綠時，
`failed` 的成交判定仍然是錯的。
...
**仍然無法驗證**：recall 需要券商真的召回借券，無法主動製造。放空本身
已於 2026-08-04 驗過（見 §6），所以擋住 D6 的不再是帳戶權限，是事件本身。
...
### 低門檻覆蓋 soak（2026-08-04 22:35 → 08-05 03:58，316 根 bar，entry_z = 0.3）

**這輪的損益完全沒有意義** —— 0.3 不是策略門檻，產生的交易策略不會做。
它的用途是讓 `strategy → sizing → price_policy → validator → fill` 被執行，
因為在真實的 1.5 門檻下那條路徑整晚跑零次（executable z 最高只到 1.16）。

**capture 97.5%，零錯誤、零駁回、零 PAUSED，驗證 63/63 全過。**
`bug #5 的修正確認有效` —— 該缺陷曾讓 `expected_price_positive` 與
`trigger_book_present` 在 UMC 腿失敗、每一筆進場被駁回。三種方向組合走過
（多進、多出、空進），`exit short_umc_long_ccf` 未補到（z 未回到 0）。

**費用模型持續運行**：399 股來回 138.37 TWD = **0.3468 TWD/股**，
每側約 $0.0053 —— 貼著 $0.005 基本費率，差額是賣出側 SEC/FINRA。
與 E4-prep 那筆 1 股（觸發 1% 上限）互補，兩個費率區間都對到真實行為。

**最有價值的量測 —— 三個 venue 的報價節奏差兩個數量級**：

| venue | 相異時戳 / 17,417 次輪詢 | 最長凍結 | 年齡 p50 | p90 | max |
|---|---|---|---|---|---|
| ibkr_umc | 16,497 | 59 次 | −0.0s | −0.0s | 64.6s |
| **fubon_ccf** | **3,596** | **99 次** | 4.2s | 20.9s | **110.9s** |
| twelvedata | **65** | **270 次** | 193.1s | 313.0s | 391.1s |

**CCF 平均每 5 次輪詢才更新一次、最久凍結 99 秒。** 這解釋了全部 7 次
`leg_timestamp_skew`（10.1–20.0s）：**CCF 時戳老不是價格錯了，是沒有成交**，
而 illiquid 商品的舊時戳意味著「什麼都沒發生」，該價格仍是最好的估計。
既有的 `ccf_book_stale_seconds = 55.0` 現在有了解釋。

**skew 門檻維持 10s，不調。** 損失僅 2.5%，而正確的修法是重新定義
「skew 對一個 5 秒才動一次的商品意味著什麼」—— 設計問題，不是參數問題。
（放寬到 15s 只多引入約 0.02 z 污染，仍遠低於 haircut，但收益太小。）

**Haircut 跨 z 範圍恆定再獲確認**：n=314，p50 **+0.180** / p90 +0.204，
量測範圍 |mid z| 0.00–1.24。與 E3 在 1.5 門檻下的 p50 0.190 / p90 0.209
幾乎相同 —— 支持「haircut 是常數、等於把門檻推高」的模型。

**出場側 haircut 首次直接觀察到**：22:38 進場的多頭價差，mid z 在 23:03
穿過 0（mid 規則會出場），可執行的 short z 到 **23:06** 才穿過 —— 延遲約 3 分鐘。
E3 標為「未建模」的出場側位移，方向與進場側一致：**進得更晚、出得也更晚**。
